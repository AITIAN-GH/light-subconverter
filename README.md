...
把订阅文件转换成 Clash / Mihomo 或 v2rayN 能直接用的格式。

轻量是它的特点：核心逻辑都在一个 `main.py` 里，除了 PyYAML 全部用 Python 标准库，不需要装一堆东西，也不用配置服务器。

最省事的用法是双击 `main.bat`，跟着菜单走。不会命令行也能用。
...
```text
http://127.0.0.1:25500/sub?target=clash&url=你的订阅文件
http://127.0.0.1:25500/sub?target=v2ray&url=你的订阅文件
```
...
> 你可以试试我的本地覆写规则  [clash_override](https://github.com/AITIAN-GH/light-subconverter/blob/main/clash_override.js)
